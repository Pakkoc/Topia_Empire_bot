...
export default function TopyShopPanelPage() {
  ...
  const { data: settings, isLoading } = useShopPanelSettingsByCurrency(guildId, "topy");
  const { data: channels } = useTextChannels(guildId);
  const createPanelMutation = useCreateTopyShopPanel(guildId);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="animate-fade-up">
        <h1 className="text-2xl md:text-3xl font-bold text-white">{topyName} 상점 패널</h1>
        <p className="text-white/50 mt-1">
          디스코드 채널에 {topyName} 상점 패널을 설치합니다
        </p>
      </div>

      {/* 안내 */}
      <div className="bg-gradient-to-r from-amber-500/10 to-yellow-500/10 rounded-2xl border border-amber-500/20 p-6">
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-xl bg-amber-500/20 flex items-center justify-center flex-shrink-0">
            <Icon icon="solar:tag-price-bold" className="h-5 w-5 text-amber-400" />
          </div>
          <div className="space-y-2">
            <h3 className="font-semibold text-white">{topyName} 상점 안내</h3>
            <ul className="text-sm text-white/60 space-y-1">
              <li>• <strong className="text-white/80">{topyName}</strong>로 구매 가능한 아이템만 표시됩니다</li>
              ...
              <li>• 구매 시 <strong className="text-white/80">{topyName}</strong>가 즉시 차감됩니다</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="font-semibold text-white">{topyName} 상점 패널 설치</h3>
            <p className="text-white/50 text-sm">디스코드 채널에 {topyName} 상점 패널을 설치합니다</p>
          </div>
        </div>
      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-white/50 text-sm">{topyName} 상점에 표시될 아이템을 관리합니다</p>
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              <p className="text-sm text-white/70">
                {topyName} 상점에 표시될 아이템은 <strong className="text-white">화폐 시스템 → 상점</strong> 메뉴에서 관리할 수 있습니다.
              </p>
              <p className="text-sm text-white/50">
                아이템의 &quot;화폐 타입&quot;을 <strong className="text-white/70">{topyName}</strong> 또는 <strong className="text-white/70">둘 다</strong>로 설정하면 이 패널에 표시됩니다.
              </p>
            </div>
          </div>
        </div>
