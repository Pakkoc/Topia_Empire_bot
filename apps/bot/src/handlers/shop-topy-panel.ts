...
const CURRENCY_TYPE = 'topy' as const;
...
function createShopEmbed(
  ...
  if (pageItems.length > 0) {
    const fields = pageItems.map((item, idx) => {
      let info = `💰 **${item.price.toLocaleString()}** ${topyName}`;
...
function createSelectMenu(
  ...
  const options = items.slice(0, 25).map((item) => {
    ...
    return {
      label: item.name,
      description: `${item.price.toLocaleString()} ${topyName}${durationInfo}`,
...
/** 토피 상점 패널 버튼 핸들러 */
export async function handleTopyShopPanelButton(
  ...
  try {
    ...
    if (items.length === 0) {
      const embed = new EmbedBuilder()
        .setColor(0xFFD700)
        .setTitle(`💰 ${topyName} 상점`)
        .setDescription(`현재 판매 중인 ${topyName} 아이템이 없습니다.`)
        .setTimestamp();
    ...
    const getComponents = () => {
      ...
      components.push(
        new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
          createSelectMenu(items, topyName, `shop_topy_panel_select_${userId}`)
        )
      );
      ...
      if (totalPages > 1) {
        components.push(
          new ActionRowBuilder<ButtonBuilder>().addComponents(
            new ButtonBuilder()
              .setCustomId(`shop_topy_panel_prev_${userId}`)
              .setLabel('◀ 이전')
              .setStyle(ButtonStyle.Secondary)
              .setDisabled(currentPage === 0),
            new ButtonBuilder()
              .setCustomId(`shop_topy_panel_next_${userId}`)
              .setLabel('다음 ▶')
              .setStyle(ButtonStyle.Secondary)
              .setDisabled(currentPage >= totalPages - 1),
            new ButtonBuilder()
              .setCustomId(`shop_topy_panel_refresh_${userId}`)
              .setLabel('🔄 새로고침')
    ...
    // 상점 Embed 생성
    const embed = createShopEmbed(items, topyName, currentPage);
    ...
    collector.on('collect', async (componentInteraction) => {
      // 페이지네이션
      if (componentInteraction.customId === `shop_topy_panel_prev_${userId}`) {
        currentPage = Math.max(0, currentPage - 1);
        await componentInteraction.update({
          embeds: [createShopEmbed(items, topyName, currentPage)],
          components: getComponents(),
        });
        return;
      }

      if (componentInteraction.customId === `shop_topy_panel_next_${userId}`) {
      ...
      if (componentInteraction.customId === `shop_topy_panel_refresh_${userId}`) {
        // 아이템 다시 조회
      ...
      // 아이템 선택
      if (componentInteraction.customId === `shop_topy_panel_select_${userId}` && componentInteraction.isStringSelectMenu()) {
        await handleItemSelection(componentInteraction, container, items, topyName);
      }
    });
    ...
    console.error('토피 상점 패널 오류:', error);
    await interaction.editReply({
      content: '상점 정보를 불러오는 중 오류가 발생했습니다.',
    });
    setTimeout(async () => {
...
function createQuantitySelectEmbed(
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: '💰 개당 가격', value: `${item.price.toLocaleString()} ${topyName}`, inline: true },
      { name: '📦 선택 수량', value: `${currentQuantity}개`, inline: true },
      { name: '💵 총 가격', value: `${totalPrice.toLocaleString()} ${topyName}`, inline: true }
    );
...
function createQuantityButtons(
  ...
  const confirmRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
      .setCustomId(`shop_topy_qty_confirm_${itemId}_${currentQuantity}_${userId}`)
      .setLabel(`${currentQuantity}개 구매하기`)
      .setStyle(ButtonStyle.Success)
      .setEmoji('✅'),
    new ButtonBuilder()
      .setCustomId(`shop_topy_qty_cancel_${userId}`)
      .setLabel('취소')
      .setStyle(ButtonStyle.Secondary)
      .setEmoji('❌')
  );
...
async function handleItemSelection(
  ...
  await interaction.update({
    embeds: [createQuantitySelectEmbed(selectedItem, topyName, currentQuantity)],
  ...
  try {
    ...
    collector.on('collect', async (componentInteraction) => {
      ...
      if (customId === `shop_topy_qty_cancel_${userId}`) {
      ...
      if (customId.startsWith(`shop_topy_qty_`) && !customId.includes('confirm') && !customId.includes('custom') && !customId.includes('cancel')) {
      ...
      if (customId === `shop_topy_qty_custom_${itemId}_${userId}`) {
        const modal = new ModalBuilder()
          .setCustomId(`shop_topy_qty_modal_${itemId}_${userId}`)
          .setTitle('수량 입력')
          .addComponents(
        ...
        try {
          const modalInteraction = await componentInteraction.awaitModalSubmit({
            filter: (i) => i.customId === `shop_topy_qty_modal_${itemId}_${userId}`,
            time: 30000,
          });
      ...
      if (customId.startsWith(`shop_topy_qty_confirm_${itemId}_`)) {
        ...
        if (!purchaseResult.success) {
          ...
          switch (purchaseResult.error.type) {
            ...
            case 'INSUFFICIENT_BALANCE':
              ...
              errorMessage = `잔액이 부족합니다.\n필요: ${required.toLocaleString()} ${topyName}\n보유: ${available.toLocaleString()} ${topyName}`;
              break;
        ...
        const successEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: '💰 지불 금액', value: `${paidAmount.toLocaleString()} ${topyName}`, inline: true },
